...
describe("Gets url from data", () => {
 test("Gets url from workId", async () => {
  const workUrl = await getWorkUrl(getWorkData({
   url: "https://archiveofourown.org/works/36667228",
  }))
 ...
 test("Fetches chapter id from url", async () => {
  ...
  expect(workUrl).toBe("https://archiveofourown.org/works/398023/chapters/659774");
 });
 ...
 test("Fetches collection from url", async () => {
  ...
  expect(workUrl).toBe("https://archiveofourown.org/collections/YJ_Prompts/works/30216801")
 });
 });