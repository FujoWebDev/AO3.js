...
async function downloadUrl(url: string) {
  try {
    ...
    // Otherwise use all segments
    const dirSegments = hasExtension ? segments.slice(0, -1) : segments;
    const dirPath = path.join(DATA_DIR, ...dirSegments.map(segment => 
      filenamify(decodeURIComponent(segment))
    ));
    await fs.mkdir(dirPath, { recursive: true });

    // If last segment has an extension, use it as filename, otherwise use index.html
    const filePath = path.join(dirPath, hasExtension ? lastSegment : 'index.html');
