...
describe("Fetches series information", () => {
  ...
  test("Fetches series object, check works", async () => {
    ...
    expect(series.works[0]).toMatchObject({
      ...
      complete: true,
      stats: { bookmarks: 208, kudos: 1324, hits: 10939 },
    });
    ...
    expect(series.works[1]).toMatchObject({
      ...
      complete: false,
      stats: { bookmarks: 644, kudos: 3032, hits: 60233 },
    });
    ...
    expect(series.works[2]).toMatchObject({
      ...
      chapters: { published: 1, total: 1 },
      complete: true,
      stats: { bookmarks: 242, kudos: 1539, hits: 12601 },
    });
    ...
    expect(series.works[3]).toMatchObject({
      ...
      chapters: { published: 1, total: 1 },
      complete: true,
      stats: { bookmarks: 81, kudos: 683, hits: 6142 },
    });
    ...
    expect(series.works[4]).toMatchObject({
      ...
      complete: false,
      stats: { bookmarks: 197, kudos: 1197, hits: 11199 },
    });
    ...
    expect(series.works[5]).toMatchObject({
      ...
      chapters: { published: 1, total: 1 },
      complete: true,
      stats: { bookmarks: 41, kudos: 346, hits: 1861 },
    });
  });
